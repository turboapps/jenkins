package org.jenkinsci.plugins.spoontrigger.schtasks;
...
public class ScheduledTasksApi {
    ...
    public static ScheduledTasksApi create(SpoonBuild build, Launcher launcher) {
        EnvVars env = build.getEnv().get();
        Charset charset = build.getCharset();
        FilePath pwd = build.getScript().get().getParent();
        return new ScheduledTasksApi(env, pwd, charset, launcher);
    }

    private ScheduledTasksApi(EnvVars env, FilePath pwd, Charset charset, Launcher launcher) {
        this.env = env;
        this.pwd = pwd;
